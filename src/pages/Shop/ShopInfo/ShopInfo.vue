  ...
  <div class="shop-info">
    <div class="info-content">
      ...
      <section class="section">
        ...
        <div class="activity">
          <div class="activity-item" v-for="(support, index) in info.supports" :key="index" :class="getClass[support.type]">
...
<script>
  ...
  export default {
    mounted () {
      // 如果没有数据直接结束
      if (!this.info.pics) { // info对象内部无值
        return
      }

    /*情况1：从别的路由跳转过来，已经有数据了*/
      this._initScroll ()
      // 纵向滚动
      /*new BScroll('.shop-info', {
        click: true
      })
      // 图片横向滚动  ul的长度必须为多个li的宽度和  才能发生滚动
      ...
      const MARGIN = 6
      ul.style.width = liWidth*len + MARGIN*(len-1) + 'px'
    ...
    data () {
      return {
        getClass: ['activity-green', 'activity-red', 'activity-orange']
      }
    },
    computed: {
      ...mapState({
